module Api::V1::ProvidersPercentagesHelper
  ...
  def self.save_providers_percentages_to_database(providers)
    ...
    providers.each do |provider|
      ...
      data["open_percentage"] = (provider["opens"].to_f / provider["delivered"].to_f).round(2) * 100
      data["click_percentage"] = (provider["clicks"].to_f / provider["delivered"].to_f).round(2) * 100
      data["spam_report_percentage"] = (provider["spam_reports"].to_f / provider["delivered"].to_f).round(2) * 100
